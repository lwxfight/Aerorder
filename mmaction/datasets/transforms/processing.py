...
@TRANSFORMS.register_module()
class PersonCrop(RandomCrop):
    def __init__(self, lazy=False):
        self.lazy = lazy

    @staticmethod
    def detect_person_bbox(img_shape, gt_bboxes):
        if len(gt_bboxes) > 0:
            person_bbox = gt_bboxes[0]  # Assuming the first bounding box corresponds to the person
            return person_bbox
        else:
            return np.array([0, 0, img_shape[1], img_shape[0]])  # Return the entire image as the bounding box if no person is detected

    def transform(self, results):
        # _lazy_init_if_proper(results, self.lazy)  # 您需要定义和导入此函数

        img_h, img_w = results['img_shape']
        gt_bboxes = results.get('gt_bboxes', [])

        person_bbox = self.detect_person_bbox((img_h, img_w), gt_bboxes)
        crop_bbox = np.round(person_bbox).astype(int)

        results['crop_bbox'] = crop_bbox

        if not self.lazy:
            if 'imgs' in results:
                results['imgs'] = self._crop_imgs(results['imgs'], crop_bbox)  # 这里您需要定义 _crop_imgs 方法或者替换成您想要使用的方法
        else:
            lazyop = results['lazy']
            if lazyop['flip']:
                raise NotImplementedError('Put Flip at last for now')

            lazyop['crop_bbox'] = crop_bbox

        return results

    def __repr__(self):
        repr_str = f'{self.__class__.__name__}(lazy={self.lazy})'
        return repr_str
...
class Resize(BaseTransform):
    ...
    def transform(self, results_all):
        ...
        _init_lazy_if_proper(results, self.lazy)
        _init_lazy_if_proper(results1, self.lazy)
        _init_lazy_if_proper(results2, self.lazy)
        _init_lazy_if_proper(results3, self.lazy)
        ...
        if 'scale_factor' not in results1:
            # print('22222')  # 输出
            results1['scale_factor'] = np.array([1, 1], dtype=np.float32)
        img_h1, img_w1 = results1['img_shape']

        if self.keep_ratio:
            # print('33333')  # 输出
            new_w1, new_h1 = mmcv.rescale_size((img_w1, img_h1), self.scale)
        else:
            # print('44444')
            new_w1, new_h1 = self.scale

        self.scale_factor1 = np.array([new_w1 / img_w1, new_h1 / img_h1],
                                     dtype=np.float32)

        results1['img_shape'] = (new_h1, new_w1)
        results1['keep_ratio'] = self.keep_ratio
        results1['scale_factor'] = results1['scale_factor'] * self.scale_factor1

        if not self.lazy:
            if 'imgs' in results1:
                # print('55555') # 输出
                results1['imgs'] = self._resize_imgs(results1['imgs'], new_w1,
                                                    new_h1)
        
        if 'scale_factor' not in results2:
            # print('22222')  # 输出
            results2['scale_factor'] = np.array([1, 1], dtype=np.float32)
        img_h2, img_w2 = results2['img_shape']

        if self.keep_ratio:
            # print('33333')  # 输出
            new_w2, new_h2 = mmcv.rescale_size((img_w2, img_h2), self.scale)
        else:
            # print('44444')
            new_w2, new_h2 = self.scale

        self.scale_factor2 = np.array([new_w2 / img_w2, new_h2 / img_h2],
                                     dtype=np.float32)

        results2['img_shape'] = (new_h2, new_w2)
        results2['keep_ratio'] = self.keep_ratio
        results2['scale_factor'] = results2['scale_factor'] * self.scale_factor2

        if not self.lazy:
            if 'imgs' in results2:
                # print('55555') # 输出
                results2['imgs'] = self._resize_imgs(results2['imgs'], new_w2,
                                                    new_h2)

        if 'scale_factor' not in results3:
            # print('22222')  # 输出
            results3['scale_factor'] = np.array([1, 1], dtype=np.float32)
        img_h3, img_w3 = results3['img_shape']

        if self.keep_ratio:
            # print('33333')  # 输出
            new_w3, new_h3 = mmcv.rescale_size((img_w3, img_h3), self.scale)
        else:
            # print('44444')
            new_w3, new_h3 = self.scale

        self.scale_factor3 = np.array([new_w3 / img_w3, new_h3 / img_h3],
                                     dtype=np.float32)

        results3['img_shape'] = (new_h3, new_w3)
        results3['keep_ratio'] = self.keep_ratio
        results3['scale_factor'] = results3['scale_factor'] * self.scale_factor3

        if not self.lazy:
            if 'imgs' in results3:
                # print('55555') # 输出
                results3['imgs'] = self._resize_imgs(results3['imgs'], new_w3,
                                                    new_h3)
...
class RandomRescale(BaseTransform):
    ...
    @staticmethod
    def detect_person_bbox(self, img_shape, gt_bboxes):
        if len(gt_bboxes) > 0:
            person_bbox = gt_bboxes[0]  # Assuming the first bounding box corresponds to the person
            return person_bbox
        else:
    ...
    def transform(self, results):
        _lazy_init_if_proper(results, self.lazy)
...
class Flip(BaseTransform):
    ...
    def transform(self, results_all):
        ...
        modality1 = results1['modality']

        flip1 = np.random.rand() < self.flip_ratio

        results1['flip'] = flip1
        results1['flip_direction'] = self.direction
        img_width1 = results1['img_shape'][1]

        if self.flip_label_map is not None and flip:
            results1['label'] = self.flip_label_map.get(results1['label'],
                                                       results1['label'])
        if not self.lazy:
            if flip1:
                if 'imgs' in results1:
                    results1['imgs'] = self._flip_imgs(results1['imgs'],
                                                      modality1)
        modality2 = results2['modality']

        flip2 = np.random.rand() < self.flip_ratio

        results2['flip'] = flip2
        results2['flip_direction'] = self.direction
        img_width2 = results2['img_shape'][1]

        if self.flip_label_map is not None and flip:
            results2['label'] = self.flip_label_map.get(results2['label'],
                                                       results2['label'])
        if not self.lazy:
            if flip2:
                if 'imgs' in results2:
                    results2['imgs'] = self._flip_imgs(results2['imgs'],
                                                      modality2)            
        
        modality3 = results3['modality']

        flip3 = np.random.rand() < self.flip_ratio

        results3['flip'] = flip3
        results3['flip_direction'] = self.direction
        img_width3 = results3['img_shape'][1]

        if self.flip_label_map is not None and flip:
            results3['label'] = self.flip_label_map.get(results3['label'],
                                                       results3['label'])
        if not self.lazy:
            if flip3:
                if 'imgs' in results3:
                    results3['imgs'] = self._flip_imgs(results3['imgs'],
                                                      modality3)
...
class CenterCrop(RandomCrop):
    ...
    def transform(self, results_all):
        ...
        results1['crop_bbox'] = crop_bbox
        results1['img_shape'] = (new_h, new_w)

        if 'crop_quadruple' not in results1:
            results1['crop_quadruple'] = np.array(
                [0, 0, 1, 1],  # x, y, w, h
                dtype=np.float32)

        old_crop_quadruple1 = results1['crop_quadruple']
        old_x_ratio1, old_y_ratio1 = old_crop_quadruple1[0], old_crop_quadruple1[1]
        old_w_ratio1, old_h_ratio1 = old_crop_quadruple1[2], old_crop_quadruple1[3]
        new_crop_quadruple1 = [
            old_x_ratio1 + x_ratio * old_w_ratio1,
            old_y_ratio1 + y_ratio * old_h_ratio1, w_ratio * old_w_ratio1,
            h_ratio * old_h_ratio1
        ]
        results1['crop_quadruple'] = np.array(
            new_crop_quadruple1, dtype=np.float32)

        if not self.lazy:
            if 'imgs' in results1:
                results1['imgs'] = self._crop_imgs(results1['imgs'], crop_bbox)
        ...
        results2['crop_bbox'] = crop_bbox
        results2['img_shape'] = (new_h, new_w)

        if 'crop_quadruple' not in results2:
            results2['crop_quadruple'] = np.array(
                [0, 0, 1, 1],  # x, y, w, h
                dtype=np.float32)

        old_crop_quadruple2 = results2['crop_quadruple']
        old_x_ratio2, old_y_ratio2 = old_crop_quadruple2[0], old_crop_quadruple2[1]
        old_w_ratio2, old_h_ratio2 = old_crop_quadruple2[2], old_crop_quadruple2[3]
        new_crop_quadruple2 = [
            old_x_ratio2 + x_ratio * old_w_ratio2,
            old_y_ratio2 + y_ratio * old_h_ratio2, w_ratio * old_w_ratio2,
            h_ratio * old_h_ratio2
        ]
        results1['crop_quadruple'] = np.array(
            new_crop_quadruple2, dtype=np.float32)

        if not self.lazy:
            if 'imgs' in results2:
                results2['imgs'] = self._crop_imgs(results2['imgs'], crop_bbox)
        
        results3['crop_bbox'] = crop_bbox
        results3['img_shape'] = (new_h, new_w)

        if 'crop_quadruple' not in results3:
            results3['crop_quadruple'] = np.array(
                [0, 0, 1, 1],  # x, y, w, h
                dtype=np.float32)

        old_crop_quadruple3 = results3['crop_quadruple']
        old_x_ratio3, old_y_ratio3 = old_crop_quadruple3[0], old_crop_quadruple3[1]
        old_w_ratio3, old_h_ratio3 = old_crop_quadruple3[2], old_crop_quadruple3[3]
        new_crop_quadruple3 = [
            old_x_ratio3 + x_ratio * old_w_ratio3,
            old_y_ratio3 + y_ratio * old_h_ratio3, w_ratio * old_w_ratio3,
            h_ratio * old_h_ratio3
        ]
        results3['crop_quadruple'] = np.array(
            new_crop_quadruple3, dtype=np.float32)

        if not self.lazy:
            if 'imgs' in results3:
                results3['imgs'] = self._crop_imgs(results3['imgs'], crop_bbox)
