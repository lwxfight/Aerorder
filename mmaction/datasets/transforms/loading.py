...
class SampleFrames(BaseTransform):
    ...
    def transform(self, results_all: dict) -> dict:
        ...
        total_frames1 = results1['total_frames']
        # if can't get fps, same value of `fps` and `target_fps`
        # will perform nothing
        fps1 = results1.get('avg_fps')
        if self.target_fps is None or not fps1:
            fps_scale_ratio1 = 1.0
        else:
            fps_scale_ratio1 = fps1 / self.target_fps
        ori_clip_len1 = self._get_ori_clip_len(fps_scale_ratio1)
        clip_offsets1 = self._sample_clips(total_frames1, ori_clip_len1)

        if self.target_fps:
            frame_inds = clip_offsets[:, None] + np.linspace(
                0, ori_clip_len - 1, self.clip_len).astype(np.int32)
        else:
            frame_inds1 = clip_offsets1[:, None] + np.arange(
                self.clip_len)[None, :] * self.frame_interval
            frame_inds1 = np.concatenate(frame_inds1)

        frame_inds1 = frame_inds1.reshape((-1, self.clip_len))
        if self.out_of_bound_opt == 'loop':
            frame_inds1 = np.mod(frame_inds1, total_frames1)
        else:
            raise ValueError('Illegal out_of_bound option.')

        start_index1 = results['start_index']
        frame_inds1 = np.concatenate(frame_inds1) + start_index1
        results1['frame_inds'] = frame_inds1.astype(np.int32)
        results1['clip_len'] = self.clip_len
        results1['frame_interval'] = self.frame_interval
        results1['num_clips'] = self.num_clips

        total_frames2 = results2['total_frames']
        # if can't get fps, same value of `fps` and `target_fps`
        # will perform nothing
        fps2 = results2.get('avg_fps')
        if self.target_fps is None or not fps2:
            fps_scale_ratio2 = 1.0
        else:
            fps_scale_ratio2 = fps2 / self.target_fps
        ori_clip_len2 = self._get_ori_clip_len(fps_scale_ratio2)
        clip_offsets2 = self._sample_clips(total_frames2, ori_clip_len2)

        if self.target_fps:
            frame_inds = clip_offsets[:, None] + np.linspace(
                0, ori_clip_len - 1, self.clip_len).astype(np.int32)
        else:
            frame_inds2 = clip_offsets2[:, None] + np.arange(
                self.clip_len)[None, :] * self.frame_interval
            frame_inds2 = np.concatenate(frame_inds2)

        frame_inds2 = frame_inds2.reshape((-1, self.clip_len))
        if self.out_of_bound_opt == 'loop':
            frame_inds2 = np.mod(frame_inds2, total_frames2)
        else:
            raise ValueError('Illegal out_of_bound option.')

        start_index2 = results['start_index']
        frame_inds2 = np.concatenate(frame_inds2) + start_index2
        results2['frame_inds'] = frame_inds2.astype(np.int32)
        results2['clip_len'] = self.clip_len
        results2['frame_interval'] = self.frame_interval
        results2['num_clips'] = self.num_clips
    
        total_frames3 = results3['total_frames']
        # if can't get fps, same value of `fps` and `target_fps`
        # will perform nothing
        fps3 = results3.get('avg_fps')
        if self.target_fps is None or not fps3:
            fps_scale_ratio3 = 1.0
        else:
            fps_scale_ratio3 = fps3 / self.target_fps
        ori_clip_len3 = self._get_ori_clip_len(fps_scale_ratio3)
        clip_offsets3 = self._sample_clips(total_frames3, ori_clip_len3)

        if self.target_fps:
            frame_inds = clip_offsets[:, None] + np.linspace(
                0, ori_clip_len - 1, self.clip_len).astype(np.int32)
        else:
            frame_inds3 = clip_offsets1[:, None] + np.arange(
                self.clip_len)[None, :] * self.frame_interval
            frame_inds3 = np.concatenate(frame_inds3)

        frame_inds3 = frame_inds1.reshape((-1, self.clip_len))
        if self.out_of_bound_opt == 'loop':
            frame_inds3 = np.mod(frame_inds3, total_frames3)
        else:
            raise ValueError('Illegal out_of_bound option.')

        start_index3 = results['start_index']
        frame_inds3 = np.concatenate(frame_inds3) + start_index3
        results3['frame_inds'] = frame_inds3.astype(np.int32)
        results3['clip_len'] = self.clip_len
        results3['frame_interval'] = self.frame_interval
        results3['num_clips'] = self.num_clips
...
class DecordInit(BaseTransform):
    ...
    def transform(self, results_all: Dict) -> Dict:
        ...
        container1 = self._get_video_reader(results1['filename'])
        results1['total_frames'] = len(container1)

        results1['video_reader'] = container1
        results1['avg_fps'] = container1.get_avg_fps()

        container2 = self._get_video_reader(results2['filename'])
        results2['total_frames'] = len(container2)

        results2['video_reader'] = container2
        results2['avg_fps'] = container2.get_avg_fps()

        container3 = self._get_video_reader(results3['filename'])
        results3['total_frames'] = len(container3)

        results3['video_reader'] = container3
        results3['avg_fps'] = container3.get_avg_fps()
...
class DecordDecode(BaseTransform):
    ...
    def transform(self, results_all: Dict) -> Dict:
        ...
        results2 = results_all[2]
        results3 = results_all[3]
        # results: {'filename': '/home/public/Hidden_Intention/Zhouz/dataset/MOD/data/bri_dahtw0_01_c_s.mp4', 'label': 3, 'sample_idx': 288, 'modality': 'RGB', 'start_index': 0, 'total_frames': 418, 'video_reader': <decord.video_reader.VideoReader object at 0x76d0c56016a0>, 'avg_fps': 29.97000378003091, 'frame_inds': array([206, 208, 210, 212, 214, 216, 218, 220, 222, 224, 226, 228, 230,
        #     232, 234, 236], dtype=int32), 'clip_len': 16, 'frame_interval': 2, 'num_clips': 1}
        # results1: {'filename': '/home/public/Hidden_Intention/Zhouz/dataset/MOD/data/bri_dahtw0_01_c_s.mp4', 'label': 3, 'sample_idx': 288, 'modality': 'RGB', 'start_index': 0, 'total_frames': 418, 'video_reader': <decord.video_reader.VideoReader object at 0x76d0c561be20>, 'avg_fps': 29.97000378003091, 'frame_inds': array([ 93,  95,  97,  99, 101, 103, 105, 107, 109, 111, 113, 115, 117,
        #     119, 121, 123], dtype=int32), 'clip_len': 16, 'frame_interval': 2, 'num_clips': 1}
        # results2: {'filename': '/home/public/Hidden_Intention/Zhouz/dataset/MOD/data/bri_dahtw0_01_c_s.mp4', 'label': 3, 'sample_idx': 288, 'modality': 'RGB', 'start_index': 0, 'total_frames': 418, 'video_reader': <decord.video_reader.VideoReader object at 0x76d0c561be20>, 'avg_fps': 29.97000378003091, 'frame_inds': array([ 93,  95,  97,  99, 101, 103, 105, 107, 109, 111, 113, 115, 117,
        #     119, 121, 123], dtype=int32), 'clip_len': 16, 'frame_interval': 2, 'num_clips': 1}
        # results3: {'filename': '/home/public/Hidden_Intention/Zhouz/dataset/MOD/data/bri_dahtw0_01_c_s.mp4', 'label': 3, 'sample_idx': 288, 'modality': 'RGB', 'start_index': 0, 'total_frames': 418, 'video_reader': <decord.video_reader.VideoReader object at 0x76d0c561be20>, 'avg_fps': 29.97000378003091, 'frame_inds': array([ 93,  95,  97,  99, 101, 103, 105, 107, 109, 111, 113, 115, 117,
        #     119, 121, 123], dtype=int32), 'clip_len': 16, 'frame_interval': 2, 'num_clips': 1}
        ...
        container1 = results1['video_reader']

        if results1['frame_inds'].ndim != 1:
            results1['frame_inds'] = np.squeeze(results1['frame_inds'])

        frame_inds1 = results1['frame_inds']
        imgs1 = self._decord_load_frames(container1, frame_inds1)

        results1['video_reader'] = None
        del container1

        results1['imgs'] = imgs1
        results1['original_shape'] = imgs1[0].shape[:2]
        results1['img_shape'] = imgs1[0].shape[:2]

        # print('results2:', results2)
        container2 = results2['video_reader']

        if results2['frame_inds'].ndim != 1:
            results2['frame_inds'] = np.squeeze(results2['frame_inds'])

        frame_inds2 = results2['frame_inds']
        # print('frame_inds2:', frame_inds2)
        imgs2 = self._decord_load_frames(container2, frame_inds2)

        results2['video_reader'] = None
        del container2

        results2['imgs'] = imgs2
        results2['original_shape'] = imgs2[0].shape[:2]
        results2['img_shape'] = imgs2[0].shape[:2]

        container3 = results3['video_reader']

        if results3['frame_inds'].ndim != 1:
            results3['frame_inds'] = np.squeeze(results3['frame_inds'])

        frame_inds3 = results3['frame_inds']
        imgs3 = self._decord_load_frames(container3, frame_inds3)

        results3['video_reader'] = None
        del container3

        results3['imgs'] = imgs3
        results3['original_shape'] = imgs3[0].shape[:2]
        results3['img_shape'] = imgs3[0].shape[:2]
